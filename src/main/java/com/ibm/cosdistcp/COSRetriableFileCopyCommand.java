...
import com.ibm.cosdistcp.COSCopyMapper.FileAction;
import com.ibm.cosdistcp.util.COSDistCpUtils;
import com.ibm.cosdistcp.util.RetriableCommand;

/**
 * This class extends RetriableCommand to implement the copy of files,
 * with retries on failure.
 */
public class COSRetriableFileCopyCommand extends RetriableCommand {

  private static Log LOG = LogFactory.getLog(COSRetriableFileCopyCommand.class);
   ...
   */
  public COSRetriableFileCopyCommand(String description, FileAction action) {
    super(description);
    this.action = action;
  }
   ...
   */
  public COSRetriableFileCopyCommand(boolean skipCrc, String description,
      FileAction action) {
  ...
  private void compareCheckSums(FileSystem sourceFS, Path source,
      ...
      throws IOException {
    if (!COSDistCpUtils.checksumsAreEqual(sourceFS, source, sourceChecksum,
        targetFS, target)) {
  ...
  private void updateContextStatus(long totalBytesRead, Mapper.Context context,
    ...
    StringBuilder message = new StringBuilder(COSDistCpUtils.getFormatter()
                .format(totalBytesRead * 100.0f / sourceFileStatus.getLen()));
    message.append("% ")
            .append(description).append(" [")
            .append(COSDistCpUtils.getStringDescriptionFor(totalBytesRead))
            .append('/')
        .append(COSDistCpUtils.getStringDescriptionFor(sourceFileStatus.getLen()))
            .append(']');
  ...
  private static ThrottledInputStream getInputStream(Path path,
    ...
    try {
      ...
      long bandwidthMB = conf.getInt(COSDistCpConstants.CONF_LABEL_BANDWIDTH_MB,
              COSDistCpConstants.DEFAULT_BANDWIDTH_MB);
