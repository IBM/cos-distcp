...
import static com.ibm.cosdistcp.COSDistCpConstants
...
public class SimpleCopyListing extends CopyListing {
  ...
  protected void validatePaths(DistCpOptions options)
    ...
    if (targetIsReservedRaw) {
      ...
      getConf().setBoolean(COSDistCpConstants.CONF_LABEL_PRESERVE_RAWXATTRS, true);
    }
      ...
      for (Path path: options.getSourcePaths()) {
        ...
        if (!explore || rootStatus.isDirectory()) {
          CopyListingFileStatus rootCopyListingStatus =
            COSDistCpUtils.toCopyListingFileStatus(sourceFS, rootStatus,
        ...
        if (explore) {
          for (FileStatus sourceStatus: sourceFiles) {
            ...
            CopyListingFileStatus sourceCopyListingStatus =
              COSDistCpUtils.toCopyListingFileStatus(sourceFS, sourceStatus,
   ...
   * @param options - Input options passed during COSDistCp invocation
    ...
    while (!pathStack.isEmpty()) {
      for (FileStatus child: getChildren(sourceFS, pathStack.pop())) {
        ...
        CopyListingFileStatus childCopyListingStatus =
          COSDistCpUtils.toCopyListingFileStatus(sourceFS, child,
  ...
  private void writeToFileListing(SequenceFile.Writer fileListWriter,
                                  ...
                                  DistCpOptions options) throws IOException {
    if (LOG.isDebugEnabled()) {
      LOG.debug("REL PATH: " + COSDistCpUtils.getRelativePath(sourcePathRoot,
    ...
    fileListWriter.append(new Text(COSDistCpUtils.getRelativePath(sourcePathRoot,
