...
class COSDistCpSync {
  ...
  static boolean sync(DistCpOptions inputOptions, Configuration conf)
    ...
    try {
      ...
      COSDistCp.LOG.warn("Failed to use snapshot diff for distcp", e);
      return false;
    } finally {
  ...
  private static Path createTargetTmpDir(DistributedFileSystem targetFs,
    ...
    final Path tmp = new Path(targetDir,
        COSDistCpConstants.HDFS_DISTCP_DIFF_DIRECTORY_NAME + COSDistCp.rand.nextInt());
  ...
  private static void deleteTargetTmpDir(DistributedFileSystem targetFs,
    ...
    try {
      if (tmpDir != null) {
    	...
    	COSDistCp.LOG.error("Unable to cleanup tmp dir: " + tmpDir, e);
    }
  }
  ...
  private static boolean checkNoChange(DistCpOptions inputOptions,
    ...
    try {
      ...
      if (!targetDiff.getDiffList().isEmpty()) {
    	  COSDistCp.LOG.warn("The target has been modified since snapshot "
    	...
    	COSDistCp.LOG.warn("Failed to compute snapshot diff on " + path, e);
    }
    return false;
  }
  ...
  static DiffInfo[] getDiffs(DistCpOptions inputOptions,
    ...
    try {
      ...
      return DiffInfo.getDiffs(sourceDiff, targetDir);
    	...
    	COSDistCp.LOG.warn("Failed to compute snapshot diff on " + sourceDir, e);
    }
    return null;
  }
