...
import com.ibm.cosdistcp.CopyListing.AclsNotSupportedException;
import com.ibm.cosdistcp.CopyListing.XAttrsNotSupportedException;
import com.ibm.cosdistcp.util.COSDistCpUtils;

/**
 * COSDistCp is the main driver-class for COS based DistCp.
 * For command-line use, COSDistCp::main() orchestrates the parsing of command-line
 * parameters and the launch of the COSDistCp job.
 * For programmatic use, a COSDistCp object can be constructed by specifying
 * options (in a DistCpOptions object), and COSDistCp::execute() may be used to
 * launch the copy-job. COSDistCp may alternatively be sub-classed to fine-tune
 * behaviour.
 */
@InterfaceAudience.Public
@InterfaceStability.Evolving
public class COSDistCp extends Configured implements Tool {
  ...
  static final Log LOG = LogFactory.getLog(COSDistCp.class);
  ...
  /**
   * Public Constructor. Creates COSDistCp object with specified input-parameters.
   ...
   * @throws Exception
   */
  public COSDistCp(Configuration configuration, DistCpOptions inputOptions) throws Exception {
  ...
  public int run(String[] argv) {
    if (argv.length < 1) {
      OptionsParser.usage();
      return COSDistCpConstants.INVALID_ARGUMENT;
    }
    String jobName = "distcp";
    if (argv.length > 1) {
      String nameOption = argv[argv.length - 2];
      if (nameOption.equals("-name")) {
        jobName = argv[argv.length - 1];
        argv = Arrays.copyOf(argv, argv.length - 2);
      }
    }
    ...
    try {
      execute(jobName);
    } catch (com.ibm.cosdistcp.CopyListing.InvalidInputException e) {
      LOG.error("Invalid input: ", e);
      return COSDistCpConstants.INVALID_ARGUMENT;
    } catch (com.ibm.cosdistcp.CopyListing.DuplicateFileException e) {
      LOG.error("Duplicate files in input path: ", e);
      return COSDistCpConstants.DUPLICATE_INPUT;
    } catch (AclsNotSupportedException e) {
      LOG.error("ACLs not supported on at least one file system: ", e);
      return COSDistCpConstants.ACLS_NOT_SUPPORTED;
    } catch (XAttrsNotSupportedException e) {
      LOG.error("XAttrs not supported on at least one file system: ", e);
      return COSDistCpConstants.XATTRS_NOT_SUPPORTED;
    } catch (Exception e) {
      LOG.error("Exception encountered ", e);
      return COSDistCpConstants.UNKNOWN_ERROR;
    }
    return COSDistCpConstants.SUCCESS;
  }
   ...
   * @throws Exception
   */
  public Job execute(String name) throws Exception {
    Job job = createAndSubmitJob(name);
   ...
   */
  public Job createAndSubmitJob(String name) throws Exception {
    ...
    try {
      ...
      if (inputOptions.shouldUseDiff()) {
        if (!COSDistCpSync.sync(inputOptions, getConf())) {
    ...
    job.getConfiguration().set(COSDistCpConstants.CONF_LABEL_DISTCP_JOB_ID, jobID);
    LOG.info("COSDistCp job-id: " + jobID);
  ...
  public void waitForJobCompletion(Job job) throws Exception {
    ...
    if (!job.waitForCompletion(true)) {
      throw new IOException("COSDistCp failure: Job " + job.getJobID()
  ...
  private void setTargetPathExists() throws IOException {
    ...
    getConf().setBoolean(COSDistCpConstants.CONF_LABEL_TARGET_PATH_EXISTS, 
        targetExists);
  }
  /**
   ...
   */
  private Job createJob(String jobName) throws IOException {
    ...
    job.setInputFormatClass(COSDistCpUtils.getStrategy(getConf(), inputOptions));
    job.setJarByClass(COSCopyMapper.class);
    configureOutputFormat(job);

    job.setMapperClass(COSCopyMapper.class);
  ...
  private void setupSSLConfig(Job job) throws IOException  {
    ...
    configuration.set(COSDistCpConstants.CONF_LABEL_SSL_CONF, sslConfigPath.getName());
    configuration.set(COSDistCpConstants.CONF_LABEL_SSL_KEYSTORE, sslConfigPath.getName());
  }
  ...
  private void addSSLFilesToDistCache(Job job,
    ...
    Path localStorePath = getLocalStorePath(sslConf,
                            COSDistCpConstants.CONF_LABEL_SSL_TRUST_STORE_LOCATION);
    job.addCacheFile(localStorePath.makeQualified(localFS.getUri(),
                                      localFS.getWorkingDirectory()).toUri());
    configuration.set(COSDistCpConstants.CONF_LABEL_SSL_TRUST_STORE_LOCATION,
                      localStorePath.getName());

    localStorePath = getLocalStorePath(sslConf,
                             COSDistCpConstants.CONF_LABEL_SSL_KEY_STORE_LOCATION);
    job.addCacheFile(localStorePath.makeQualified(localFS.getUri(),
                                      localFS.getWorkingDirectory()).toUri());
    configuration.set(COSDistCpConstants.CONF_LABEL_SSL_KEY_STORE_LOCATION,
                                      localStorePath.getName());
  ...
  private void configureOutputFormat(Job job) throws IOException {
    ...
    if (inputOptions.shouldPreserve(DistCpOptions.FileAttribute.ACL)) {
      COSDistCpUtils.checkFileSystemAclSupport(targetFS);
    }
    if (inputOptions.shouldPreserve(DistCpOptions.FileAttribute.XATTR)) {
      COSDistCpUtils.checkFileSystemXAttrSupport(targetFS);
    }
    if (inputOptions.shouldAtomicCommit()) {
      ...
      if (!COSDistCpUtils.compareFs(targetFS, workFS)) {
  ...
  private Path createMetaFolderPath() throws Exception {
    ...
    configuration.set(COSDistCpConstants.CONF_LABEL_META_FOLDER, metaFolderPath.toString());    
    return metaFolderPath;
  }

  /**
   * Main function of the COSDistCp program. Parses the input arguments (via OptionsParser),
  ...
  public static void main(String argv[]) {
    int exitCode;
    try {
      COSDistCp distCp = new COSDistCp();
      ...
      LOG.error("Couldn't complete COSDistCp operation: ", e);
      exitCode = COSDistCpConstants.UNKNOWN_ERROR;
    }
  ...
  private static class Cleanup implements Runnable {
    private final COSDistCp distCp;

    Cleanup(COSDistCp distCp) {
      this.distCp = distCp;
    }
